...
def quiz_detail(request, slug):
    ...
    try:
        ...
        return render_to_response('quiz/quiz_score.html', { 'quiz': quiz, 'score': score, 'questions': questions, 'corrent_anwser': corrent_anwser },context_instance=RequestContext(request))
    except Score.DoesNotExist:
        form_list = quiz_forms(quiz)
        return render_to_response('quiz/quiz_detail.html', 
                                  { 'form_list': form_list, 'quiz': quiz }, 
...
def process_quiz(request, slug):
    ...
    if request.method == 'POST':
        ...
        for form in form_list:
            if form.is_correct():
                try:
                    question = Question.objects.get(question=form.question)
                    right.append(form.question)
                except:
                    question = None
        if right:
            score.corrent_anwsers.add(*right)
            score.save()
